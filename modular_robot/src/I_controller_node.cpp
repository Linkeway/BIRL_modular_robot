...
public:
    bool initialize()
    {
        ...
        if(has_pos_controller_)
        {
            ...
            cmd_pos_sub_ = nh_.subscribe("joint_group_position_controller/command", 1, &ControlModeAdapter::cmd_pos_cb, this);
        }
        if(has_vel_controller_)
        {
            ...
            cmd_vel_sub_ = nh_.subscribe("joint_group_velocity_controller/command", 1, &ControlModeAdapter::cmd_vel_cb, this);
        }
        ...
        update_rate_ = 100;    //[hz]
        timer_ = nh_.createTimer(ros::Duration(1/update_rate_), &ControlModeAdapter::update, this);
...
{
    ros::init(argc, argv, "I_control_mode_adapter_node");
    ros::AsyncSpinner spinner(0);
    spinner.start();

    ControlModeAdapter* cma = new ControlModeAdapter();

    if (!cma->initialize())
    {
        ROS_ERROR("Failed to initialize ControlModeAdapter");
        return -1;
    }
